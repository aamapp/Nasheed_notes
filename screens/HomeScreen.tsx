...
export const HomeScreen: React.FC<HomeScreenProps> = ({ 
  ...
  const handleDeleteClick = (e: React.MouseEvent, id: string) => {
    ...
    // Small delay to let the menu close visually and prevent UI blocking immediately
    setTimeout(() => {
          ...
          filteredNasheeds.map(nasheed => {
            ...
            return (
              <div 
                ...
                // Added z-index handling for active menu state to prevent overlap issues
                className={`group relative bg-card hover:bg-slate-700/50 border border-slate-700/50 rounded-xl p-4 transition-all active:scale-[0.99] cursor-pointer ${isMenuOpen ? 'z-30' : 'z-0'}`}
              >
                <div className="flex justify-between items-start mb-2">
                  ...
                  {/* Dropdown Menu */}
                  {isMenuOpen && (
                    <div className="absolute right-2 top-10 z-20 w-36 bg-slate-800 border border-slate-700 rounded-lg shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-100">
                      <button 
                        onClick={(e) => handleEditClick(e, nasheed)}
                        className="w-full text-left px-4 py-3 text-sm text-slate-200 hover:bg-slate-700 flex items-center gap-2 border-b border-slate-700/50"
                      >
